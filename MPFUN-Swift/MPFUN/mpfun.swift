...
//    mpb24x = 2.d0 ** 24, mpr24x = 0.5d0 ** 24, mpb13x = 2.d0 ** 13)
    ...
    static let mpb13x = pow(2.0, 13.0)
...
//        lb = Int(b[0])   ?? Not Used
    ...
    static func mpmuld (_ a : MPRNumber, _ b : Double, _ c : inout MPRNumber, _ mpnw : Int) {
        ...
        for i in 3...na + 2 {
            ...
            a2 = a[i+1] - a1
            dc = a1 * b2 + a2 * b1
            c1 = mpbdx * Double(mprdx) * dc
            c2 = dc - c1
            d[i] = d[i] + Double(mprdx) * (a1 * b1 + c1)
            d[i+1] = a2 * b2 + c2
        }
    ...
    static func mpmuld40 (_ a : MPRNumber, _ b : Double, _ c : inout MPRNumber, _ mpnw : Int) {
        ...
        if (t2 == abs (b)) {
            ...
            print ("*** MPMULD40: DP value has more than 40 significant bits:",
                "and thus very likely represents an unintended loss of accuracy.",
                "Fix the issue, or else use functions mpprodd, mpquotd, mpreald or mpcmplxdc.",
                "See documentation for details.")
            mpabrt (83)
...
//        write (iu, "(4f18.0)") (a[i], i = 0, na + 5)
    ...
    static func mpsqrt (_ a : MPRNumber, _ b : inout MPRNumber, _ mpnw : Int) {
        ...
        mpmdc (a, &t1, &n, mpnw)
        n2 = -n / 2
        t2 = sqrt (t1 * pow(2.0, Double(n + 2 * n2)))
        t1 = 1 / t2
        mpdmc (t1, n2, &s2, mpnw)
        mpdmc (1, 0, &s3, mpnw)
...
//        return
    ...
    static func mpdivx (_ a : MPRNumber, _ b : MPRNumber, _ c : inout MPRNumber, _ mpnw : Int)  {
        ...
        if (na == 0) {
            // Shouldn"t this return 0 in c? - Note: original assigned to b
            c[1] = Double(mpnw)
            c[2] = 0
            c[3] = 0
            return
...
//        return
    ...
    static func mpfftcr (_ iss : Int, _ m : Int, _ n : Int, _ nsq : Int, x : MPRComplex, y : inout MPRNumber) {
        ...
        n1 = Int(pow(2.0, Double(m / 2)))
        n2 = n / 2
        n21 = n2 + 1
        n4 = n / 4
        ai = Complex64(0, 1)  // (0, 1, mprknd)
    ...
    static func mpfftrc (_ iss: Int, _ m: Int, _ n: Int, _ nsq: Int, _ x : MPRNumber, _ y : inout MPRComplex) {
        ...
        var k, ku, m, mx, n, nsq, n1, n2, n21, n4 : Int
        var dc1 = MPRComplex(repeating: Complex64(), count: n/2)
    ...
    static func mpfft1 (_ iss: Int, _ m: Int, _ n1: Int, _ n2: Int, _ x : MPRComplex, _ y : MPRComplex) {
...
//    return
    ...
    static func mpfft2 (_ iss: Int, _ ns: Int, _ m: Int, _ n: Int, _ x : MPRComplex, _ y : MPRComplex) {
...
//    return
    ...
    static func mpfft3 (_ iss: Int, _ l: Int, _ ns: Int, _ m: Int, _ n: Int, _ x: MPRComplex, _ y: MPRComplex) {
...
//    return
    ...
    static func mplconv (_ iq: Int, _ n: Int, _ nsq: Int, _ a: MPRNumber, _ b: MPRNumber, _ c: MPRNumber) {
...
//    return
    ...
    static func mpqmc (_ a: MPRNumber, _ n: Int, _ b: inout MPRNumber, _ mpnw: Int) {
